...
const DoctorDeletePage = () => {
 ...
 // 处理更新排班信息
    ...
    const handleAddDoctor = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // 2. 创建医生的排班信息
        // 使用 moment 转换为 UTC 时间，并格式化为后端期望的格式
        const startTime = moment.utc().startOf('day').add(8, 'hours').format('YYYY-MM-DD HH:mm:ss');
        const endTime = moment.utc().startOf('day').add(38, 'days').add(16, 'hours').format('YYYY-MM-DD HH:mm:ss');

        const scheduleRequestData = {
            doctorId: doctorData.doctorId,
            appointmentPrice: 50, // 添加默认挂号费用
            startTime: startTime,
            endTime: endTime
        };
